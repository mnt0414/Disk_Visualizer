...
export async function scanFolder(path: string): Promise<ScanSummary> { requireDesktop(); return invoke<ScanSummary>('scan_folder', { path: normalizePath(path) }) }
export async function startScan(path: string): Promise<ScanJobSnapshot> { requireDesktop(); return invoke<ScanJobSnapshot>('start_scan', { path: normalizePath(path) }) }
export async function getScanStatus(id: number): Promise<ScanJobSnapshot> { requireDesktop(); return invoke<ScanJobSnapshot>('get_scan_status', { id }) }
export async function pauseScan(id: number): Promise<ScanJobSnapshot> { requireDesktop(); return invoke<ScanJobSnapshot>('pause_scan', { id }) }
