...
export async function listSavedScans(): Promise<SavedScan[]> {
  requireDesktop()
  return invoke<SavedScan[]>('list_saved_scans')
}
export async function listCacheEntries(
  scanId: number,
  limit = 100,
  offset = 0,
): Promise<CacheEntryDetail[]> {
  requireDesktop()
  return invoke<CacheEntryDetail[]>('list_cache_entries', {
    scanId,
    limit,
    offset,
  })
...
export async function deleteSavedScan(id: number): Promise<void> {
  requireDesktop()
  return invoke('delete_saved_scan', { id })
}
export async function checkScanIndex(): Promise<boolean> {
  requireDesktop()
  return invoke<boolean>('check_scan_index')
}
