...
const largeItems = [['Premiere Projects','~/Movies/Premiere Projects','68.4 GB'],['DaVinci CacheClip','~/Movies/DaVinci Resolve','41.7 GB'],['Photos Library.photoslibrary','~/Pictures','32.6 GB'],['Xcode.app','/Applications','18.2 GB']]
const caches = [['Adobe Premiere Pro','メディアキャッシュ','24.8 GB','高'],['DaVinci Resolve','Render Cache','18.3 GB','高'],['Google Chrome','ブラウザキャッシュ','8.6 GB','高'],['Blender','シミュレーションキャッシュ','5.4 GB','中']]
...
function DataTable({ title, rows, eyebrow = 'モックデータ' }: { title: string; rows: string[][]; eyebrow?: string }) {
  return <section className="panel mock-panel"><div className="mock-header"><span className="eyebrow">{eyebrow}</span><h2>{title}</h2></div>{rows.length === 0 ? <p className="scan-empty">表示できる項目はありません</p> : <table><thead><tr><th>名前</th><th>分類／場所</th><th>サイズ</th><th>状態</th></tr></thead><tbody>{rows.map((row) => <tr key={`${row[0]}-${row[1]}`}><td><strong>{row[0]}</strong></td><td>{row[1]}</td><td>{row[2]}</td><td>{row[3] ?? '確認済み'}</td></tr>)}</tbody></table>}</section>
}

function StorageMap() { return <div className="mock-layout"><section className="panel mock-map" aria-label="容量の分布"><div className="map-a">制作データ<br />184 GB</div><div className="map-b">アプリ<br />112 GB</div><div className="map-c">写真<br />86 GB</div><div className="map-d">アプリキャッシュ<br />64 GB</div></section><section className="panel mock-panel"><div className="mock-header"><span className="eyebrow">選択中</span><h2>Macintosh HD</h2></div><ul className="mock-list">{['制作データ 184 GB','アプリケーション 112 GB','写真 86 GB','アプリキャッシュ 64 GB'].map((item) => <li key={item}>{item}</li>)}</ul></section></div> }

function Comparison() { return <div className="mock-stack"><section className="mock-metrics"><article><span>使用量の変化</span><strong>+22.3 GB</strong></article><article><span>増加した項目</span><strong>1,284</strong></article><article><span>減少した項目</span><strong>426</strong></article></section><DataTable title="カテゴリ別の変化" rows={[["制作データ","7日間","+18.4 GB","増加"],["アプリキャッシュ","7日間","+6.8 GB","増加"],["ダウンロード","7日間","-4.2 GB","減少"]]} /></div> }
