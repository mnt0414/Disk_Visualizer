...
## Volume boundary policy

- スキャン開始時にルートのボリュームIDを取得する。
- ディレクトリのボリュームIDがルートと異なる場合、その配下へは降りない。
- ボリュームIDを取得できない環境では推測で除外せず、読み取り可能な範囲を走査する。
- 境界で除外したディレクトリは`skipped_count`へ計上する。
